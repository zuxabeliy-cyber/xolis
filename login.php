...
body{background:radial-gradient(at 20% 8%, rgba(31,174,118,.15) 0px, transparent 45%),radial-gradient(at 88% 92%, rgba(201,162,75,.08) 0px, transparent 42%),radial-gradient(at 50% 50%, rgba(11,61,44,.14) 0px, transparent 60%), #0d100c; background-attachment:fixed;}
.login-card{position:relative;background:linear-gradient(160deg,rgba(23,34,25,.65),rgba(13,16,12,.65));backdrop-filter:blur(16px) saturate(130%);-webkit-backdrop-filter:blur(16px) saturate(130%);box-shadow:0 0 90px -10px rgba(31,174,118,.2), 0 25px 60px -20px rgba(0,0,0,.75);animation:fadeUp .6s cubic-bezier(.2,.8,.2,1) both;transition:.45s cubic-bezier(.2,.8,.2,1);}
.login-card::before{content:'';position:absolute;inset:0;border-radius:inherit;padding:1px;background:linear-gradient(160deg,rgba(201,162,75,.3),transparent 45%,transparent 65%,rgba(31,174,118,.25));-webkit-mask:linear-gradient(#fff 0 0) content-box,linear-gradient(#fff 0 0);-webkit-mask-composite:xor;mask-composite:exclude;pointer-events:none}
.btn-glow{box-shadow:0 4px 24px -4px rgba(31,174,118,.45);transition:.25s cubic-bezier(.2,.8,.2,1)}
.btn-glow:hover{box-shadow:0 10px 36px -4px rgba(31,174,118,.65);transform:translateY(-2px)}
.btn-glow:active{transform:scale(.97)}
input{transition:.2s}
.logo-ring{position:relative;width:180px;height:165px;margin:0 auto}
.logo-ring::before{content:'';position:absolute;inset:-6px;border-radius:50%;background:conic-gradient(from 0deg,#1fae76,transparent 30%,transparent 70%,#c9a24b);animation:glowPulse 2.4s ease-in-out infinite;filter:blur(10px)}
...
<div class="logo-ring"><img src="logo_full.png" class="relative w-full h-full mx-auto object-contain drop-shadow-[0_0_22px_rgba(31,174,118,.4)]" style="animation:floaty 3.2s ease-in-out infinite"></div>
...
<button type="button" onclick="openForm()" class="w-full bg-[#1fae76] hover:bg-[#37c98b] text-white p-4 rounded-2xl font-black tracking-widest btn-glow">KIRISH</button>
</div>

<div id="formWrap" class="<?php echo $err ? 'open' : ''; ?>">
<form method="post" class="space-y-3 text-left">
<input name="login" required placeholder="Login" class="w-full p-4 rounded-2xl border border-white/10 outline-none focus:border-[#1fae76] focus:shadow-[0_0_0_3px_rgba(31,174,118,.15)] text-white placeholder-white/30">
<input name="pass" type="password" required placeholder="Parol" class="w-full p-4 rounded-2xl border border-white/10 outline-none focus:border-[#1fae76] focus:shadow-[0_0_0_3px_rgba(31,174,118,.15)] text-white placeholder-white/30">
<button class="w-full bg-[#1fae76] hover:bg-[#37c98b] text-white p-4 rounded-2xl font-black tracking-widest btn-glow">KIRISH</button>
</form>
</div>
