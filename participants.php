...
list($mc,$mp)=monthCond($ym,'p.created_at'); if($mc){ $sqlBase.=$mc; foreach($mp as $x) $pr[]=$x; }
if($q!==''){
...
<div>
<h1 class="font-black text-xl flex items-center gap-2"><?php echo icon('list','w-5 h-5'); ?> Ro'yxat <?php echo $totalCount; ?> ta <?php if($totalWithPromo!=$totalCount): ?><span class="text-[#7c6cff] text-sm">(promo bilan <?php echo $totalWithPromo; ?> ta)</span><?php endif; ?></h1>
<p class="text-[10px] text-white/30 mt-1">Sahifa <?php echo $page; ?> / <?php echo $totalPages; ?> • Har sahifada <?php echo $perPage; ?> tadan</p>
</div>
<a href="api.php?action=export_csv" class="btn btn-light btn-sm">⬇️ Excel</a>
</div>
<?php echo monthSelectorHtml($ym,($q!==''?['q'=>$q]:[])); ?>
<div class="card p-3 mb-3"><form class="flex gap-2"><input type="hidden" name="ym" value="<?php echo htmlspecialchars($ym); ?>"><input name="q" value="<?php echo htmlspecialchars($q); ?>" placeholder="Ism, nomer, 4 xona, operator yoki tarif" class="flex-1 p-3 rounded-xl bg-black/50 border border-white/10 text-white outline-none"><button class="btn btn-primary">Qidir</button><?php if($q!==''): ?><a href="participants.php?ym=<?php echo urlencode($ym); ?>" class="btn btn-ghost btn-sm">✕ Tozalash</a><?php endif; ?></form></div>

<?php if($totalPages>1): ?>
<div class="flex gap-1 flex-wrap mb-3">
<?php if($page>1): ?><a href="?ym=<?php echo urlencode($ym); ?>&q=<?php echo urlencode($q); ?>&page=<?php echo $page-1; ?>" class="bg-white/10 border border-white/10 px-3 py-2 rounded-lg text-xs">⬅ Oldingi</a><?php endif; ?>
<?php for($i=1;$i<=$totalPages;$i++): if($i==1 || $i==$totalPages || ($i>=$page-2 && $i<=$page+2)): ?>
<a href="?ym=<?php echo urlencode($ym); ?>&q=<?php echo urlencode($q); ?>&page=<?php echo $i; ?>" class="px-3 py-2 rounded-lg text-xs font-bold <?php echo $i==$page?'bg-white text-black':'bg-white/5 border border-white/10 text-white/60'; ?>"><?php echo $i; ?></a>
<?php elseif($i==$page-3 || $i==$page+3): ?><span class="px-2 py-2 text-white/20 text-xs">...</span><?php endif; endfor; ?>
<?php if($page<$totalPages): ?><a href="?ym=<?php echo urlencode($ym); ?>&q=<?php echo urlencode($q); ?>&page=<?php echo $page+1; ?>" class="bg-white/10 border border-white/10 px-3 py-2 rounded-lg text-xs">Keyingi ➡</a><?php endif; ?>
</div>
<?php endif; ?>
...
<?php if($isSuper): ?>
<span id="pstate-<?php echo $r['id']; ?>" class="text-[10px] <?php echo $r['is_paid']?'text-[#7c6cff]':'text-white/30'; ?>"><?php echo $r['is_paid']?"O'YINDA":'BAZADA'; ?></span>
<?php if(!empty($r['promo_count']) && $r['promo_count']==2): ?><span class="ml-1 text-[9px] bg-[#7c6cff]/15 text-[#7c6cff] px-1.5 py-0.5 rounded-full font-bold">🎁 x2</span><?php endif; ?>
...
<button type="button" onclick="markState(<?php echo $r['id']; ?>,1)" class="px-2 py-0.5 rounded-md text-[9px] font-bold border border-[#7c6cff]/20 bg-[#7c6cff]/10 text-[#7c6cff] hover:bg-[#7c6cff]/20">→ O'YINGA</button>
</div>
<?php else: ?>
<span class="text-[10px] <?php echo $r['is_paid']?'text-[#7c6cff]':'text-white/30'; ?>"><?php echo $r['is_paid']?"O'YINDA":'BAZADA'; ?></span>
<?php if(!empty($r['promo_count']) && $r['promo_count']==2): ?><span class="ml-1 text-[9px] bg-[#7c6cff]/15 text-[#7c6cff] px-1.5 py-0.5 rounded-full font-bold">🎁 x2</span><?php endif; ?>
<?php endif; ?>
</td>
<td class="text-xs"><?php if($r['status']=='pending'): ?><span class="px-2 py-1 rounded-full bg-[#7c6cff]/10 text-[#7c6cff] border border-[#7c6cff]/20 text-[10px] font-bold">⏳ Kutilmoqda</span><?php elseif($r['status']=='rejected'): ?><span class="px-2 py-1 rounded-full bg-red-500/10 text-red-300 border border-red-500/20 text-[10px] font-bold">❌ Rad etildi</span><?php else: ?><span class="px-2 py-1 rounded-full bg-white/5 text-white/50 border border-white/10 text-[10px] font-bold">✅ Tasdiqlandi</span><?php endif; ?></td>
...
<a href="edit.php?id=<?php echo $r['id']; ?>" class="text-[#7c6cff] mr-3">✏️</a><a href="delete.php?id=<?php echo $r['id']; ?>" onclick="return confirm('<?php echo $r['status']=='pending' && !$isSuper ? "Kutilmoqdagi yuborishingizni bekor qilasizmi?" : "Ochir?"; ?>')" class="text-red-400">🗑</a>
...
function markState(id, val){
 ...
 .then(function(d){
  if(d.ok){
   ...
   if(el){ el.textContent = d.is_paid=='1' ? "O'YINDA" : 'BAZADA'; el.className = 'text-[10px] ' + (d.is_paid=='1' ? 'text-[#7c6cff]' : 'text-white/30'); }
  }
 }).catch(function(){});
}
</script>
