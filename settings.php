...
<?php if($msg): ?><div class="card p-4 mb-4 bg-[#1fae76]/10 border border-[#1fae76]/20 text-[#1fae76] text-sm font-bold"><?php echo htmlspecialchars($msg); ?></div><?php endif; ?>
...
<label class="flex items-center gap-2 cursor-pointer bg-black/30 p-3 rounded-xl"><input type="checkbox" name="baza_sends_channel" value="1" <?php echo $baza_sends_channel?'checked':''; ?> class="w-5 h-5 accent-[#1fae76]"><span class="text-sm font-bold">BAZAGA bosilganda kanalga yuborilsin</span></label>
</div>

<div class="border-t border-white/5 pt-4">
<label class="text-xs font-bold text-[#1fae76] tracking-widest">📝 1. Yangi ishtirokchi qo'shilganda kanalga boradigan xabar</label>
<p class="text-[10px] text-white/30 mt-1 mb-2">Kodlar: {diller} {ism} {nomer} {operator} {tarif}</p>
<textarea name="template" rows="6" class="w-full p-3 rounded-xl bg-black/50 border border-white/10 text-white font-mono text-sm outline-none focus:border-[#1fae76]/50"><?php echo htmlspecialchars($template); ?></textarea>
</div>
...
<label class="flex items-center gap-2 cursor-pointer"><input type="checkbox" name="daily_limit_enabled" value="1" <?php echo $daily_limit_enabled?'checked':''; ?> class="w-5 h-5 accent-[#1fae76]"><span class="text-sm font-bold">Yoqish / O'chirish</span></label>
...
<div class="bg-[#1fae76]/5 border border-[#1fae76]/10 p-3 rounded-xl text-xs mt-2 whitespace-pre">🎉 Yangi ishtirokchi!
1. Diller: {diller}
2. Ism: {ism}
3. Nomer: {nomer}
4. Operator: {operator}
