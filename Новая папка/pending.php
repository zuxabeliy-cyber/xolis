...
<h1 class="font-black text-xl flex items-center gap-2"><?php echo icon('hourglass','w-5 h-5'); ?> Kutilmoqda <span class="text-[#1fae76]"><?php echo count($rows); ?></span> ta</h1>
<div class="flex gap-2">
<?php if(count($rows)>0): ?>
<form method="post" onsubmit="return confirm('Kutilmoqdagi HAMMA nomerlarni tasdiqlaysizmi? Bu amalni ortga qaytarib bo\'lmaydi.')"><input type="hidden" name="approve_all" value="1"><button class="bg-[#1fae76] text-white px-4 py-2 rounded-xl text-xs font-black btn-glow">✅ Hammasini tasdiqlash</button></form>
...
<div class="card p-4 card-hover border-l-2 border-l-[#1fae76]/40">
<div class="flex flex-wrap justify-between items-start gap-3">
<div>
<div class="flex items-center gap-2 flex-wrap"><b class="text-base"><?php echo htmlspecialchars($r['name']); ?></b><span class="text-[10px] px-2 py-1 rounded-full bg-[#1fae76]/10 text-[#1fae76] border border-[#1fae76]/20 font-bold">⏳ KUTILMOQDA</span><span class="text-[10px] px-2 py-1 rounded-full <?php echo $r['is_paid']?'bg-white text-black':'bg-white/10 text-white/40'; ?>"><?php echo $r['is_paid']?"O'YINDA":'BAZADA'; ?></span></div>
...
<form method="post" onsubmit="return confirm('Tasdiqlaysizmi?')"><input type="hidden" name="approve_id" value="<?php echo $r['id']; ?>"><button class="bg-[#1fae76] text-white px-3 py-2 rounded-xl text-xs font-black">✅ Tasdiqlash</button></form>
...
<td><?php echo $h['status']=='approved' ? '<span class="text-[10px] px-2 py-1 rounded-full bg-[#1fae76]/10 text-[#1fae76] border border-[#1fae76]/20 font-bold">✅ TASDIQLANDI</span>' : '<span class="text-[10px] px-2 py-1 rounded-full bg-red-500/10 text-red-300 border border-red-500/20 font-bold">❌ RAD ETILDI</span>'; ?></td>
...
<?php if($toast): ?><div id="toast" class="fixed top-4 left-1/2 -translate-x-1/2 z-[999] max-w-sm w-[92%] shadow-2xl"><div class="bg-white/5 border border-[#1fae76]/20 text-[#1fae76] p-3 rounded-xl text-sm font-bold text-center"><?php echo $toast; ?></div></div>
