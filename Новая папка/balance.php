...
function growthBadge($pct){
 $up = $pct>=0;
 $cls = $up ? 'text-[#1fae76] bg-[#1fae76]/10 border-[#1fae76]/20' : 'text-red-300 bg-red-500/10 border-red-500/20';
 $arrow = $up ? '↑' : '↓';
...
<div class="card p-6 mb-4 border-[#1fae76]/25 text-center relative overflow-hidden">
...
<div><p class="text-white/30 text-[10px]">TO'LANGAN</p><p class="font-bold text-[#1fae76]"><?php echo fmtSum($myPaid); ?></p></div>
...
<div class="w-full h-2.5 bg-white/5 rounded-full overflow-hidden"><div class="h-full bg-gradient-to-r from-[#1fae76] to-[#c9a24b]" style="width:<?php echo $pct; ?>%"></div></div>
</div>
<?php endif; ?>
<?php endif; ?>
</div>
...
<a href="balance.php?range=<?php echo $k; ?>" class="px-3 py-1.5 rounded-lg text-xs font-bold border <?php echo ($range===$k || ($k==='all' && $range==='' && !$from)) ? 'bg-[#1fae76] text-white border-[#1fae76]' : 'bg-white/5 text-white/60 border-white/10'; ?>"><?php echo $lbl; ?></a>
<?php endforeach; ?>
</div>
...
<?php if($isSuper): ?><a href="balance.php?export=csv&scope=all&<?php echo $qsFilter; ?>" class="bg-[#1fae76] text-white px-3 py-2 rounded-lg text-xs font-bold">📊 Excel</a><?php else: ?><a href="balance.php?export=csv&scope=dealer&id=<?php echo $u['id']; ?>&<?php echo $qsFilter; ?>" class="bg-[#1fae76] text-white px-3 py-2 rounded-lg text-xs font-bold">📊 Excel</a><?php endif; ?>
</form>
</div>
</div>
...
<b class="text-[#1fae76] text-sm"><?php echo fmtSum($d['balance']); ?></b>
</div>
<?php endforeach; ?>
</div>
<div class="card p-4">
...
<b class="text-[#1fae76] text-sm"><?php echo fmtSum($t['balance']); ?></b>
</div>
<?php endforeach; ?>
</div>
</div>
...
<p class="font-black text-lg text-[#1fae76] mt-1"><?php echo fmtSum($ob['balance']); ?> <span class="text-xs text-white/30">so'm</span></p>
...
<div><p class="font-black text-lg text-[#1fae76]"><?php echo fmtSum($r['balance']); ?></p><p class="text-[10px] text-white/30">so'm</p></div>
<span class="text-white/30 chevron">▾</span>
</div>
</div>
<?php if($r['monthly_target']>0): $pct=min(100,round($r['balance']/$r['monthly_target']*100)); ?>
<div class="mt-2"><div class="flex justify-between text-[10px] text-white/30 mb-1"><span>Maqsad</span><span><?php echo $pct; ?>%</span></div><div class="w-full h-1.5 bg-white/5 rounded-full overflow-hidden"><div class="h-full bg-gradient-to-r from-[#1fae76] to-[#c9a24b]" style="width:<?php echo $pct; ?>%"></div></div></div>
<?php endif; ?>
...
<td class="text-center font-bold text-[#1fae76]"><?php echo fmtSum($b['balance']); ?></td>
</tr>
<?php endforeach; ?>
</tbody></table>
</div>
...
<tr class="border-b border-white/5"><td class="p-3 text-xs text-white/50"><?php echo $p['created_at']; ?></td><td class="text-xs text-white/50"><?php echo htmlspecialchars($p['note']); ?></td><td class="text-center font-bold text-[#1fae76]"><?php echo fmtSum($p['amount']); ?></td></tr>
<?php endforeach; ?>
</tbody></table>
</div>
<?php endif; ?>
...
new Chart(document.getElementById('monthlyChart'), {
 ...
 data: {
  ...
  datasets: [{ label: "So'm", data: monthlyData.map(m => m.balance), backgroundColor: 'rgba(16,185,129,.55)', borderRadius: 6 }]
 },
 ...
 card.addEventListener('click', () => {
  ...
  fetch('balance.php?ajax=operator_detail&op='+encodeURIComponent(op)+'&<?php echo $qsFilter; ?>')
   .then(r=>r.json()).then(d=>{
    detail.innerHTML = (d.rows||[]).map(t=>`<div class="flex justify-between"><span class="text-white/50">${t.tarif_name}</span><b class="text-[#1fae76]">${Number(t.balance).toLocaleString('ru-RU')}</b></div>`).join('') || '<span class="text-white/30">Ma\'lumot yo\'q</span>';
  ...
  fetch('balance.php?ajax=dealer_detail&id='+id+'&<?php echo $qsFilter; ?>')
   .then(r=>r.json()).then(d=>{
    ...
    html += `<div class="grid grid-cols-3 gap-2 mb-4 text-center">
      ...
      <div class="bg-black/30 rounded-xl p-2"><p class="text-[10px] text-white/30">TO'LANGAN</p><p class="font-bold text-sm text-[#1fae76]">${Number(d.paid).toLocaleString('ru-RU')}</p></div>
    ...
    html += `<form method="post" class="flex gap-2 mb-2">
      ...
      <button class="bg-[#1fae76] text-white px-3 rounded-lg text-xs font-bold whitespace-nowrap">💵 To'ladim</button>
    </form>`;
    ...
    if((d.list||[]).length){
     ...
     d.list.forEach(p=>{ html += `<tr class="border-t border-white/5"><td class="p-1">${p.name}</td><td>${p.pretty_phone}</td><td class="text-center">${p.tarif_name}</td><td class="text-center text-[#1fae76]">${Number(p.price).toLocaleString('ru-RU')}</td><td class="text-center text-white/30">${p.created_at}</td></tr>`; });
    ...
    if((d.payments||[]).length){
     ...
     d.payments.forEach(p=>{ html += `<div class="flex justify-between text-xs bg-black/20 p-2 rounded-lg"><span class="text-white/40">${p.created_at} ${p.note?('• '+p.note):''}</span><b class="text-[#1fae76]">${Number(p.amount).toLocaleString('ru-RU')}</b></div>`; });
     html += '</div>';
    }
